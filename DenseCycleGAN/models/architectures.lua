...
function defineG(input_nc, output_nc, ngf, which_model_netG, nz, arch)
  ...
  elseif which_model_netG == "densenet_5blocks" then netG = defineG_densenet_blocks(input_nc, output_nc, ngf, 5)
  elseif which_model_netG == "densenet_6blocks" then netG = defineG_densenet_blocks(input_nc, output_nc, ngf, 6)
  elseif which_model_netG == "densenet_7blocks" then netG = defineG_densenet_blocks(input_nc, output_nc, ngf, 7)
  elseif which_model_netG == "densenet_8blocks" then netG = defineG_densenet_blocks(input_nc, output_nc, ngf, 8)
...
function DenseConnectLayerStandard(nChannels, growthRate, concat)
   local net = nn.Sequential()

   local p = 0
   net:add(nn.SpatialReflectionPadding(1, 1, 1, 1))
   net:add(ShareGradInput(nn.SpatialBatchNormalization(nChannels), 'first'))
   net:add(nn.ReLU(true))   
   net:add(nn.SpatialConvolution(nChannels, growthRate, 3, 3, 1, 1, p, p))
...
local function addDenseBlock(model, nChannels, growthRate, N)
  for i = 1, N-1 do 
     model:add(DenseConnectLayerStandard(nChannels, growthRate, true))   
     nChannels = nChannels + growthRate
  end
  model:add(DenseConnectLayerStandard(nChannels, growthRate, false))   
end


function defineG_densenet_blocks(input_nc, output_nc, ngf, n_layers_dense)
  ...
  -- e1
  netG:add(nn.SpatialReflectionPadding(p, p, p, p))
    :add(nn.SpatialConvolution(input_nc, ngf, f, f, 1, 1))
    :add(normalization(ngf))
    :add(nn.ReLU(true))
  
  -- e2
  netG:add(nn.SpatialConvolution(ngf, ngf*2, ks, ks, 2, 2, 1, 1))
    :add(normalization(ngf*2))
    :add(nn.ReLU(true))

  -- e3
  netG:add(nn.SpatialConvolution(ngf*2, ngf*4, ks, ks, 2, 2, 1, 1))
    :add(normalization(ngf*4))
    :add(nn.ReLU(true))

  -- d1
  addDenseBlock(netG, ngf*4, ngf*4, n_layers_dense)

  -- d2
  netG:add(nn.SpatialFullConvolution(ngf*4, ngf*2, ks, ks, 2, 2, 1, 1,1,1))
    :add(normalization(ngf*2))
    :add(nn.ReLU(true))

  -- d3
  netG:add(nn.SpatialFullConvolution(ngf*2, ngf, ks, ks, 2, 2, 1, 1,1,1))
    :add(normalization(ngf))
    :add(nn.ReLU(true))

  -- d4
  netG:add(nn.SpatialReflectionPadding(p, p, p, p))
    :add(nn.SpatialConvolution(ngf, output_nc, f, f, 1, 1))
    :add(nn.Tanh())
